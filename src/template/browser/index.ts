...
const appendStyle = (anchor: Exclude<BrowserTemplateNode, string>, keyword: string, css: string): void => {
...
export class BrowserTemplate extends Template {
    ...
    protected constructor(text: string[], preparsed: ASTNode[] | null, inputs: BrowserTemplateNode[], delimiters?: Delimiters) {
        super(text, preparsed, delimiters)
        this.inputs = inputs
    }

    static makeFromNode(input: BrowserTemplateNode, delimiters?: Delimiters): BrowserTemplate {
        return BrowserTemplate.makeFromNodes([input], delimiters)
    }

    static makeFromNodes(inputs: BrowserTemplateNode[], delimiters?: Delimiters): BrowserTemplate {
    ...
    renderToNodes(tagRenderer: TagRenderer): void {
        super.render(tagRenderer, (outputs: string[]) => outputs.forEach((text: string, index: number) => setText(this.inputs[index], text)))
    }
    ...
    appendStyle(input: Exclude<BrowserTemplateNode, string>, keyword: string, css: string): void {
