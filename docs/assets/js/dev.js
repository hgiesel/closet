(function () {
    ...
    ***************************************************************************** */
    ...
    var spliceSlice = function (text, lend, rend, add) {
        if (add === void 0) { add = ''; }
    ...
    var getNewOffset = function (replacement, lend, rend) {
        return replacement.length - (rend - lend);
    };
    var replaceAndGetOffset = function (text, repl, lend, rend) {
        return [
    ...
    var postfixTraverse = function (baseText, rootTag, filterProcessor) {
        var tagReduce = function (_a, tag) {
            ...
            var _e = __read(filterOutput.ready
                ? replaceAndGetOffset(modText, filterOutput.result, lend, rend)
                : [modText, 0], 2), newText = _e[0], newOffset = _e[1];
            // going UP
    ...
    var PriorityQueue = /** @class */ (function () {
        ...
        PriorityQueue.prototype.siftDown = function () {
            ...
            while ((left(node) < this.size() && this.greater(left(node), node)) ||
                ...
                var maxChild = (right(node) < this.size() && this.greater(right(node), left(node))) ? right(node) : left(node);
        ...
        PriorityQueue.prototype.forEach = function (proc) {
            var nextItem = this.pop();
    ...
    var deferredComparator = function (x, y) { return x.priority < y.priority; };
    var DeferredApi = /** @class */ (function () {
        function DeferredApi() {
            this._deferred = new Map();
        }
        DeferredApi.prototype.register = function (keyword, procedure, priority) {
        ...
        DeferredApi.prototype.registerIfNotExists = function (keyword, proc, prio) {
            if (prio === void 0) { prio = 50; }
            if (!this.has(keyword)) {
                this.register(keyword, proc, prio);
            }
        };
        DeferredApi.prototype.has = function (keyword) {
            return this._deferred.has(keyword);
        };
        DeferredApi.prototype.unregister = function (keyword) {
        ...
        DeferredApi.prototype.executeEach = function () {
            ...
            var prioQueue = new PriorityQueue(deferredComparator);
            prioQueue.push.apply(prioQueue, __spread(this._deferred.values()));
            prioQueue.forEach(function (_a) {
                var procedure = _a.procedure, keyword = _a.keyword;
                return procedure.apply(void 0, __spread([keyword], args));
            });
        };
        return DeferredApi;
    }());
    ...
    var debugRecipe = function (filterApi) {
        ...
        filterApi.register('key', (function (_a) {
            var key = _a.key;
            return key;
        }));
        filterApi.register('test', (function (_t, _a) {
            var deferred = _a.deferred;
            deferred.registerIfNotExists('foobar', function () {
                console.log('foobar');
            }, 50);
            deferred.registerIfNotExists('mehbar', function () {
                console.log('mehbar');
            }, 20);
            deferred.registerIfNotExists('echbar', function () {
                console.log('echbar');
            }, 20);
            deferred.registerIfNotExists('gehbar', function () {
                console.log('gehbar');
            }, 200);
            return '';
        }));
